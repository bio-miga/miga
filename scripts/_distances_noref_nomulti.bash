...
if [[ $(miga project_info -P "$PROJECT" -m type) != "clade" ]] ; then
  ...
  while [[ -e "$CLADES/$CLASSIF/miga-project.1.medoids" ]] ; do
    ...
    CLASSIF="$CLASSIF/miga-project.1.subcl-$AAI_MED"
    echo "$AAI_MED	$MAX_AAI	$CLASSIF" >> "$DATASET.aai-medoids.tsv"
  done
else
  ...
  while [[ -e "$CLADES/$CLASSIF/miga-project.1.medoids" ]] ; do
    ...
    CLASSIF="$CLASSIF/miga-project.1.subcl-$ANI_MED"
    echo "$ANI_MED	$MAX_ANI	$CLASSIF" >> "$DATASET.ani-medoids.tsv"
  done
